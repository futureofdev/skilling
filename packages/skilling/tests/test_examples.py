...
`hello-skilling` teaches the format; `test-drive` exists to exercise it. Its job is to touch
...
EXERCISER = REPO_ROOT / "examples" / "test-drive"
...
def test_its_shape_is_what_the_manifest_says(drive: Course) -> None:
    assert drive.id == "test-drive"
    assert drive.phase_count == 2
    assert drive.lesson_count == 4
    assert [p.number for p in drive.phases] == [1, 2]
    assert [p.lesson_count for p in drive.phases] == [2, 2]


def test_every_lesson_file_exists_at_its_derived_path(drive: Course) -> None:
    missing = [lesson.coordinate for lesson in drive.lessons() if not lesson.path.is_file()]
    assert missing == []


def test_it_stays_small_enough_to_deliver_in_one_sitting(drive: Course) -> None:
...
def test_homework_lands_at_every_phase_boundary(drive: Course) -> None:
    with_homework = {lesson.coordinate for lesson in drive.lessons() if lesson.homework}
    boundaries = {phase.lessons[-1].coordinate for phase in drive.phases}
    assert with_homework == boundaries, "every phase ends in homework, and nothing else carries it"


def test_every_badge_is_registered_and_reachable(drive: Course) -> None:
    registered = drive.manifest.badge_ids
...
def test_every_lesson_uses_structured_objectives(drive: Course) -> None:
    for lesson in drive.lessons():
...
def test_objective_ids_are_unique_within_each_lesson(drive: Course) -> None:
    for lesson in drive.lessons():
        fm = md.parse_lesson(lesson.path).frontmatter
        assert fm
...
def test_both_objective_kinds_appear(drive: Course) -> None:
...
def test_only_practice_objectives_carry_verification(drive: Course) -> None:
    for lesson in drive.lessons():
        fm = md.parse_lesson(lesson.path).frontmatter
        assert fm
        for objective in fm.objectives:
...
def test_verify_coverage_is_honest(drive: Course) -> None:
...
def test_a_runtime_with_no_capabilities_may_settle_nothing_here(drive: Course) -> None:
...
def test_observation_reaches_exactly_the_verified_practice_objectives(drive: Course) -> None:
    from skilling import delivery as runtime
...
def test_about_only_names_questions_that_exist(drive: Course) -> None:
    for lesson in drive.lessons():
        parsed = md.parse_lesson(lesson.path)
...
def test_every_absence_carries_a_reason(drive: Course) -> None:
    absences = 0
    for lesson in drive.lessons():
        fm = md.parse_lesson(lesson.path).frontmatter
        assert fm
...
def test_the_final_lesson_declares_why_it_ends_quietly(drive: Course) -> None:
    last = drive.lesson_at(drive.coordinates[-1])
    assert last
...
def test_ceremony_carries_facts_a_tutor_may_not_invent(drive: Course) -> None:
    ceremony = drive.manifest.ceremony
...
def test_every_phase_has_a_pronoun_free_highlight(drive: Course) -> None:
    for phase in drive.phases:
...
def test_the_share_template_renders_with_derived_counts(drive: Course) -> None:
    text = cer.share_text(drive, None, drive.phase_at(1))
    assert text
...
def test_the_quiz_grammar_holds_across_every_question(drive: Course) -> None:
    questions = 0
    for lesson in drive.lessons():
...
def test_an_asset_is_referenced_and_resolves(drive: Course) -> None:
...
def test_prerequisites_form_an_unbroken_chain(drive: Course) -> None:
    coordinates = drive.coordinates
    for i, coordinate in enumerate(coordinates):
        lesson = drive.lesson_at(coordinate)
        assert lesson
        fm = md.parse_lesson(lesson.path).frontmatter
        assert fm
...
def test_it_is_licensed_for_reuse(drive: Course) -> None:
    assert drive.manifest.license == "CC-BY-4.0"
...
def test_every_phase_has_an_overview(drive: Course) -> None:
    for phase in drive.phases:
        assert phase.overview_path is not None, phase.number


def test_a_full_walk_completes_cleanly_and_stays_small(tmp_path: Path, drive: Course) -> None:
    from skilling import delivery as runtime
    from skilling.store import LOCAL_LEARNER, FileProgressStore

    store = FileProgressStore(tmp_path / "state")
    record, revision = runtime.load_or_create(store, drive, LOCAL_LEARNER)
    for lesson in drive.lessons():
        outcome = runtime.complete_lesson(store, drive, record, revision, lesson)
        record, revision = outcome.record, outcome.revision

    assert len(record.completed) == 4
    assert len(record.skills_unlocked) == 3
    assert drive.percent_complete(record.completed) == 100.0
    size = (tmp_path / "state" / drive.id / "record.yaml").stat().st_size
